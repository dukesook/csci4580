...
Enhancements:
		...
		- Removed debugging functions from previous lab
		- Added ASTNode structure to union
		- Preserved each grammar rule from previous labs
		- #include "ast.h"
		- Created AST nodes for each grammar rule
		- Created ASTprint function to print the AST
		- Defined %type for each non-terminal
		- Defined %type <node> for AST nodes
		- Defined %type <datatype> for data types
		- Defined %type <operator> for operators
		- Passed up AST nodes using $$ = $1, $$ = $2, etc.
		- Called ASTCreateNode() function to create AST nodes
		- Set node type, operator, datatype, name, value, and child nodes for each AST node
		- Created main function to call yyparse() and print the AST
		- Implemented Graduate Student Required Rules:
			- Func_Prototype
			- Continue_Stmt
			- Break_Stmt
...
/* Rule #22 */
Variable: T_ID 										  { $$ = ASTCreateNode(A_VARIABLE); // Create variable node
																			$$->name = $1; }
